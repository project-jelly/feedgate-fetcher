...
- `INDEX (next_fetch_at) WHERE status <> 'dead'` — 스케줄러의 "곧 fetch할
  대상" 조회 최적화. broken 포함, dead만 제외하는 부분 인덱스.
...
- 14/20 피드가 ETag 또는 Last-Modified 지원 (70%). 조건부 요청(`If-None-Match`/`If-Modified-Since`) 구현으로 해당 피드는 304 응답 시 본문 수신 없이 스킵됨
